...
class Swbignews {
	...
	public static function load_class( $class , $module = null ) {
		...
		if( !class_exists( $class ) ) {
			$path = SWBIGNEWS_THEME_DIR . '/framework/';
		...
		return class_exists( 'Swbignews_' . $class );
	}
	...
	public static function get_config( $name, $field = NULL ) {
		...
		if( false == $setting ) {
			if( file_exists( SWBIGNEWS_FRAMEWORK_DIR . '/config.php' ) ) {
				$setting = require( SWBIGNEWS_FRAMEWORK_DIR . '/config.php' );
			}
		}
	...
	public static function get_params( $name, $field = NULL ) {
		static $params = false;
		if( false == $params ) {
			if( file_exists( SWBIGNEWS_FRAMEWORK_DIR . '/params.php' ) ) {
				$params = require( SWBIGNEWS_FRAMEWORK_DIR . '/params.php' );
			}
		}
	 ...
	 * @return string
	 */
	public static function get_option( $name, $field = null,$def ='' ) {
		global $swbignews_options;
	
		if( $field ) {
			return ( isset( $swbignews_options[$name][$field] ) ) ? $swbignews_options[$name][$field] : '';
		}
		if( isset ($swbignews_options[$name] ) ) {
			return $swbignews_options[$name];
		}
		return $def ;
	}
}