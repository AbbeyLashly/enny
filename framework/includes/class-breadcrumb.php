...
/**
 * Swbignews_Breadcrumb class.
 *
 * @class Swbignews_Breadcrumb
 * @author Swlabs
 * @since 1.0
 */
class Swbignews_Breadcrumb {
	...
	public function get_breadcrumb() {
		return apply_filters( 'swbignews_get_breadcrumb', $this->crumbs, $this );
	}
	...
	private function add_crumbs_404() {
		$this->add_crumb( Swbignews_Translate::_swt( 'Error 404' ) );
	}
	...
	private function add_crumbs_single( $post_id = 0, $permalink = '' ) {
		...
		if ( 'post' != get_post_type( $post ) ) {
			...
			$cat = '';
			$post_options = get_post_meta( $post->ID, 'shw_page_options', true);
	...
	private function add_crumbs_tag() {
		...
		$this->add_crumb( Swbignews_Translate::_swt( 'Posts tagged').sprintf( ' &ldquo;%s&rdquo;', single_tag_title( '', false ) ), get_tag_link( $queried_object->term_id ) );
	}
	...
	private function add_crumbs_author() {
		...
		$this->add_crumb(Swbignews_Translate::_swt( 'Author').': '.esc_html( $userdata->display_name ));
	}
	...
	private function search_trail() {
		if ( is_search() ) {
			$this->add_crumb( Swbignews_Translate::_swt( 'Search results for').sprintf( ' &ldquo;%s&rdquo;' , get_search_query() ), remove_query_arg( 'paged' ) );
		}
	}
	...
	private function paged_trail() {
		if ( get_query_var( 'paged' ) ) {
			$this->add_crumb( Swbignews_Translate::_swt( 'Page' ).' '.get_query_var( 'paged' ) );
		}
	}
}
